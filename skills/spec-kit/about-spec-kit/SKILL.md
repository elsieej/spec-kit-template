...
thay đổi...) nằm ở `docs/spec-kit-conventions.md` — đọc khi cần chi tiết hơn phần tóm tắt dưới
đây (xem "Điều kiện tiên quyết" bên dưới nếu file này chưa có trong dự án).
...
`templates/spec-kit-conventions.md` đi kèm sẵn cạnh `SKILL.md` này. Kiểm tra
`docs/spec-kit-conventions.md` đã tồn tại chưa — nếu chưa có, copy nguyên trạng file trên vào đó
(không hỏi user, không tạo subfolder) trước khi trả lời bất kỳ câu hỏi nào bên dưới; đây là hành
động duy nhất skill này ghi vào dự án, mọi việc khác chỉ đọc/giải thích.
...
2. Review từng tầng xong (set `status: approved`) mới sang tầng sau — không nhảy cấp (xem
   `docs/spec-kit-conventions.md` mục 2, nguyên tắc chung #2).
...
- **`docs/spec-kit-conventions.md`** — quy tắc xuyên suốt (naming/ID, `status`, glossary-link,
  ma trận lan truyền thay đổi, commit format...); mỗi skill mang theo 1 bản seed và tự scaffold
  file này nếu dự án chưa có, nên chỉ tồn tại đúng 1 bản trong dự án.

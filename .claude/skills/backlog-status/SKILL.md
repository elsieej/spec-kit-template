...
Kit này không theo dõi sprint/lịch triển khai — việc đó thuộc repo triển khai (xem
`AGENTS.md`, "Làm việc đa repo"). Nếu user hỏi "sprint nào đang chạy", trả lời rằng spec-kit
không theo dõi khái niệm này, chỉ theo dõi backlog ở mức Epic/Feature/User Story.
...
1. **Backlog hiện có**: đọc frontmatter mọi file trong
   ...
   - `draft` → liệt kê theo nhóm (đây là phần có thể bắt đầu thực hiện).
...
2. **Cây phân cấp Epic → Feature → User Story**: đọc frontmatter mọi file trong
   ...
   `parent_epic` (Feature), `parent_feature` (User Story):
   ...
   - **Phát hiện item mồ côi ở đúng cấp của nó**: Epic không có `parent_business_requirement`
     trỏ tới BR tồn tại, hoặc Feature/US không match được vào cây (parent không tồn tại)

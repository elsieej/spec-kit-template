...
> BR/UR/FR/glossary/EPIC/FEAT/US/OQ/MEET, không phải cơ chế riêng). Sau khi scaffold, chỉ có
> **1 bản duy nhất** tồn tại trong dự án đích, tại `docs/spec-kit-conventions.md` — không phải
> 5 bản trùng lặp nằm rải trong từng skill đã cài.
>
> Sửa nội dung ở đây (bản canonical của repo nguồn) rồi chạy `scripts/sync-conventions.sh` từ
> gốc repo để đồng bộ sang 5 bản seed trong `skills/spec-kit/*/templates/`, không tự `cp` tay.
